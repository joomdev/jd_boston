...
?><div class="grid-x grid-margin-x">
    <?php if (empty($data['allMails']) && empty($data['search']) && empty($data['tag']) && empty($data['status']) && $data['type'] == 'standard') { ?>
		<div class="grid-x cell text-center">
			...
			<div class="medium-4 small-12 cell">
				<a href="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl').'&task=edit&step=editEmail&from=-1&id='.acym_escape($data['campaignID'])); ?>" class="button expanded" id="acym__templates__choose__create__empty">
    ...
    <?php } else { ?>
		...
		<div class="medium-auto cell">
            ...
            echo acym_select($data["allTags"], 'mailchoose_tag', acym_escape($data["tag"]), 'class="acym__templates__filter__tags"', 'value', 'name'); ?>
		</div>
		...
		<div class="grid-x xlarge-auto large-shrink text-center cell acym__templates__choose__type-templates">
            ...
            echo acym_switchFilter($switchfilter, acym_escape($data['type']), 'mailchoose_type');
            ?>
		</div>
