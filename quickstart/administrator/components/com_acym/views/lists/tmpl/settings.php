	...
	<form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl')); ?>" method="post" name="acyForm" data-abide novalidate>
		...
		<div class="grid-x grid-margin-x margin-bottom-0">
            ...
            <?php if (!empty($data['listInformation']->id)) { ?>
				<p class="cell large-2 medium-4 small-6 text-center" id="acym__list__settings__list-id"><?php echo acym_translation('ACYM_LIST_ID'); ?> : <b class="acym__color__blue"><?php echo acym_escape($data['listInformation']->id); ?></b></p>
			...
			<div class="cell grid-x large-2 medium-4 small-4 acym__list__settings__active">
                <?php echo acym_switch('list[active]', acym_escape($data['listInformation']->active), acym_translation('ACYM_ACTIVE'), [], 'shrink', 'auto', 'tiny margin-0'); ?>
			</div>
			<div class="cell large-6">
				<label>
