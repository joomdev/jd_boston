...
?><form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl').'&id='.intval($data['listId'])); ?>" method="post" name="acyForm">
	<div id="acym__welcome" class="acym__content">
		...
		<div class="grid-x margin-bottom-0">
            ...
            <?php if (empty($data['welcomeMails']) && empty($data['search'])) { ?>
				<h1 class="cell text-center acym__listing__empty__title">
                    <?php
                    echo acym_translation_sprintf('ACYM_NO_TEMPLATE', strtolower(acym_translation('ACYM_WELCOME_MAIL')));
                    $return = acym_completeLink('lists&task=edit&step=welcome&id='.$data['listId'].'&edition=1');
                    $url = acym_completeLink('mails&task=edit&step=editEmail&type=welcome&type_editor=acyEditor&return='.urlencode($return));
                    ?>
					<a href="<?php echo $url; ?>"> <?php echo acym_translation('ACYM_CREATE_ONE'); ?></a>
				</h1>
            <?php } else { ?>
				...
				<div class="large-6 medium-8 cell">
                    <?php echo acym_filterSearch($data['search'], 'welcome_search', 'ACYM_SEARCH_TEMPLATE'); ?>
				</div>
            <?php } ?>
		</div>
        <?php if (!empty($data['welcomeMails'])) { ?>
			<div class="grid-x grid-padding-x grid-padding-y grid-margin-x grid-margin-y xxlarge-up-6 large-up-4 medium-up-3 small-up-1">
                <?php acym_selectTemplates($data['welcomeMails'], $data['selectedWelcome'], 'welcome', $data['listId']); ?>
			</div>
		...
		<div class="cell grid-x">
			...
			<div class="cell medium-auto grid-x text-right">
				...
				<button data-task="save" data-step="unsubscribe" type="submit" class="cell medium-shrink button margin-bottom-0 acy_button_submit">
	...
	<input type="hidden" name="id" value="<?php echo acym_escape($data['listId']); ?>">
    <?php acym_formOptions(true, 'edit', 'welcome'); ?>
</form>
