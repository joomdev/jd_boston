...
?><form id="acym_form" action="<?php echo acym_completeLink(acym_getVar('cmd', 'ctrl').'&id='.acym_escape($data['listId'])); ?>" method="post" name="acyForm">
	<div id="acym__unsubscribe" class="acym__content">
		...
		<div class="grid-x margin-bottom-0">
            ...
            <?php if (empty($data['unsubscribeMails']) && empty($data['search'])) { ?>
				<h1 class="cell text-center acym__listing__empty__title">
                    <?php
                    echo acym_translation_sprintf('ACYM_NO_TEMPLATE', strtolower(acym_translation('ACYM_UNSUBSCRIBE')));
                    $return = acym_completeLink('lists&task=edit&step=unsubscribe&id='.$data['listId'].'&edition=1');
                    $url = acym_completeLink('mails&task=edit&step=editEmail&type=unsubscribe&type_editor=acyEditor&return='.urlencode($return));
                    ?>
        ...
        <?php if (!empty($data['unsubscribeMails'])) { ?>
			<div class="grid-x grid-padding-x grid-padding-y grid-margin-x grid-margin-y xxlarge-up-6 large-up-4 medium-up-3 small-up-1">
                <?php acym_selectTemplates($data['unsubscribeMails'], $data['selectedUnsubscribe'], 'unsubscribe', $data['listId']); ?>
			</div>
		...
		<div class="cell grid-x">
			...
			<div class="cell medium-auto grid-x text-right">
				...
				<button data-task="save" data-step="listing" type="submit" class="cell medium-shrink button medium-margin-bottom-0 margin-right-1 acy_button_submit">
    ...
    <?php acym_formOptions(true, 'edit', 'unsubscribe'); ?>
</form>
