		...
		<div class="cell grid-x grid-margin-x grid-margin-y">
			<div class="<?php echo in_array($data['field']->id, [1, 2]) ? '' : 'medium-6 '; ?>cell grid-x acym__fields__edit__field acym__content grid-margin-x">
				...
				<label class="cell large-5"><?php echo acym_translation('ACYM_FIELD_TYPE'); ?>
                    <?php
                    echo acym_select($data['fieldType'], 'field[type]', $data['field']->type, 'class="acym__fields__edit__select"'.((!empty($data['field']->id) && in_array($data['field']->id, [1, 2])) ? 'disabled' : ''), 'value', 'name');
                    ?>
				</label>
				<div class="cell grid-x large-6 margin-top-1">
                    <?php echo acym_switch('field[active]', $data['field']->active, acym_translation('ACYM_ACTIVE'), [], 'shrink', 'auto', 'tiny margin-0'); ?>
				</div>
                ...
                <?php if (empty($data['field']->id) || $data['field']->id != 2) { ?>
					<div class="cell grid-x large-6  margin-top-1 acym__fields__change" id="acym__fields__required">
                        <?php echo acym_switch('field[required]', $data['field']->required, acym_translation('ACYM_REQUIRED'), [], 'shrink', 'auto', 'tiny margin-0', 'required_error_message'); ?>
					</div>
                <?php } ?>
				...
				<div class="cell grid-x margin-top-1 acym__fields__change" id="acym__fields__editable-user-creation">
                    <?php echo acym_switch('field[option][editable_user_creation]', empty($data['field']->option) ? 1 : $data['field']->option->editable_user_creation, acym_translation('ACYM_EDITABLE_USER_CREATION'), [], 'shrink', 'auto', 'tiny margin-0'); ?>
				</div>
				<div class="cell grid-x margin-top-1 acym__fields__change" id="acym__fields__editable-user-modification">
                    <?php echo acym_switch('field[option][editable_user_modification]', empty($data['field']->option) ? 1 : $data['field']->option->editable_user_modification, acym_translation('ACYM_EDITABLE_USER_MODIFICATION'), [], 'shrink', 'auto', 'tiny margin-0'); ?>
				</div>
				...
				<label class="cell margin-top-1 acym__fields__change large-5" id="acym__fields__html-tag-cat"><?php echo acym_translation('ACYM_HTML_TAG_CATEGORIES'); ?>
                    <?php
                    echo acym_radio(['div' => 'Div', 'fieldset' => 'Fieldset'], 'acym__fields__edit__html-tag-cat', 'div');
                    ?>
				</label>
                            ...
                            <?php if (empty($data['field']->value)) { ?>
								<div class="grid-x cell acym__fields__value__sortable acym__content margin-bottom-1 grid-margin-x">
									...
									<div class="cell medium-2">
                                        <?php echo acym_select(['n' => acym_translation('ACYM_NO'), 'y' => acym_translation('ACYM_YES')], 'field[value][disabled][]', 'n', 'class="acym__fields__edit__select"', 'value', 'name'); ?>
									</div>
								</div>
                            <?php } else {
                                $i = 0;
                                foreach ($data['field']->value as $value) { ?>
									<div class="grid-x cell acym__fields__value__sortable acym__content margin-bottom-1 grid-margin-x">
										...
										<div class="cell medium-2">
                                            <?php echo acym_select(['n' => acym_translation('ACYM_NO'), 'y' => acym_translation('ACYM_YES')], 'field[value][disabled][]', $value->disabled, 'class="acym__fields__edit__select"', 'value', 'name'); ?>
										</div>
				...
				<div class="cell large-11 grid-x acym__fields__change acym__content" id="acym__fields__from-db">
					...
					<label class="cell margin-top-1 medium-5"><?php echo acym_translation('ACYM_TABLES'); ?>
                        <?php echo acym_select(empty($data['field']->fieldDB->tables) ? [] : $data['field']->fieldDB->tables, 'fieldDB[table]', empty($data['field']->fieldDB->table) ? '' : $data['field']->fieldDB->table, 'class="acym__fields__edit__select"', 'value', 'name'); ?>
					</label>
					<label class="cell margin-top-1 medium-5"><?php echo acym_translation('ACYM_VALUE'); ?>
                        <?php echo acym_select(empty($data['field']->fieldDB->columns) ? [] : $data['field']->fieldDB->columns, 'fieldDB[value]', empty($data['field']->fieldDB->value) ? '' : $data['field']->fieldDB->value, 'class="acym__fields__edit__select acym__fields__database__columns"', 'value', 'name'); ?>
					</label>
					<div class="medium-1"></div>
					<label class="cell margin-top-1 medium-5"><?php echo acym_translation('ACYM_TITLE'); ?>
                        <?php echo acym_select(empty($data['field']->fieldDB->columns) ? [] : $data['field']->fieldDB->columns, 'fieldDB[title]', empty($data['field']->fieldDB->title) ? '' : $data['field']->fieldDB->title, 'class="acym__fields__edit__select acym__fields__database__columns"', 'value', 'name'); ?>
					</label>
					<label class="cell margin-top-1 medium-4 margin-right-1"><?php echo acym_translation('ACYM_WHERE'); ?>
                        <?php echo acym_select(empty($data['field']->fieldDB->columns) ? [] : $data['field']->fieldDB->columns, 'fieldDB[where]', empty($data['field']->fieldDB->where) ? '' : $data['field']->fieldDB->where, 'class="acym__fields__edit__select acym__fields__database__columns"', 'value', 'name'); ?>
					</label>
					...
					<label class="cell margin-top-1 medium-5"><?php echo acym_translation('ACYM_SORT_ORDERING'); ?>
                        <?php echo acym_select(['asc' => 'ASC', 'desc' => 'DESC'], 'fieldDB[sort_order]', empty($data['field']->fieldDB->sort_order) ? '' : $data['field']->fieldDB->sort_order, 'class="acym__fields__edit__select"', 'value', 'name'); ?>
					</label>
				</div>
			</div>
			<div class="medium-6 cell grid-x acym__fields__edit__back-front<?php echo in_array($data['field']->id, [1, 2]) ? ' is-hidden' : ''; ?>">
