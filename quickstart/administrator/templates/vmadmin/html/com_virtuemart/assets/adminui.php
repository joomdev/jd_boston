...
class AdminUIHelper_override {
	...
	static function startAdminArea($vmView,$selectText = 'COM_VIRTUEMART_DRDOWN_AVA2ALL') {
		...
		<div class="menu-wrapper<?php echo $hideMenu ?>" id="menu-wrapper">
			<?php if(!empty($vmView->langList)){ ?>
				<div class="vm-lang-list-container" style="display:none;">
	...
	static function showAdminMenu($vmView) {
		...
		$menuItems = AdminUIHelper_override::_getAdminMenu ( $moduleId );
		$app = JFactory::getApplication();
		$isSite = $app->isSite();

		?>
		<!-- <div id="admin-ui-menu" class="admin-ui-menu"> -->
        <div id="menuspecial" class="menuspecial white">
			<?php
			$modCount = 1;
			...
			foreach ( $menuItems as $item ) {
					...
					if ($link ['name'] == '-') {
						...
						|| $target || $link ['view']=='about' || $link ['view']=='virtuemart') {
							...
							if($link['view'] == $currentview)
							{
								...
								$activegroup = "active";
								$activegroup_style = "display:block;";
							}
						...
						<li class="'.$activeclass.'">
							<a href="'.$url.'" '.$target.' >
								<span class="vmicon-wrapper hasTip" title="'. vmText::_ ( $link ['name'] ).'"><span class="'.$link ['icon_class'].'"></span></span>
				...
				if(!empty($html)){
					?>
                    <li class="<?php echo $activegroup; ?>"><a href="#">
					...
					<span class="menu-title-wrapper">
						...
						<ul class="submenuspecial" style="<?php echo $activegroup_style; ?>">
							<?php echo $html ?>
						</ul>
                    </li>
